...
import { BookOpen, Link as LinkIcon, Lightbulb, Search } from "lucide-react";

const container = {
  hidden: { opacity: 0 },
  show: {
    opacity: 1,
    transition: { staggerChildren: 0.1 } // Staggered entrance
  }
};
    ...
    >
      {items.map((item,index) => (
        <motion.div
          ...
          variants={itemAnim}
          whileHover={{ y: -5, transition: { duration: 0.2 } }} // Subtle hover lift
        ...
        >
          <div className="flex justify-between items-start mb-4">
            <div className="p-2 bg-indigo-50 dark:bg-indigo-900/30 rounded-lg text-indigo-600">
              {item.type === 'note' ? <BookOpen size={20} /> : <LinkIcon size={20} />}
            </div>
          ...
          <div className="mt-4 flex flex-wrap gap-2">
            {item.tags.map(tag => (
              <span key={tag} className="px-2 py-1 bg-zinc-100 dark:bg-zinc-800 rounded-md text-[10px] font-bold uppercase text-zinc-500">
                #{tag}
              </span>
            ))}
          </div>
